...
- Latest local pricing fix, uncommitted: TCG API documentation uses both `Foil` and `Holofoil` for the holo finish. The matcher now treats them as the same finish and uses an exact catalog rarity when available, without rejecting older-set rarity wording. All 28 tests passed. Await Andrew's live test and commit/push authorization.
- Latest local full-identity lookup, uncommitted: the app resolves the scanner's set name or abbreviation to TCG API's numeric set ID (cached for 24 hours), then searches TCG with `q` (name), `game=pokemon`, `set_id`, catalog `rarity`, and `printing` where documented. It verifies collector number locally and only narrows by a special stamp if TCG's product name/set contains it. Rarity falls back safely for older wording. All 29 tests passed, including a regression that confirms set ID, rarity, and foil filters are sent. Await Andrew's live test and commit/push authorization.
- Current uncommitted files are `netlify/functions/cardhedge.mjs`, `tests/pokemon-pricing.test.mjs`, and this handoff. Do not discard them. No commit/push authorization has been given for this batch.
- Update: the full-identity TCG matching work above was committed and pushed to `main` as `27e0628` — Match Pokemon cards using full TCG identity. That commit was confirmed to match `origin/main`. The pending-file statement immediately above is obsolete except for any later handoff edits.
- Andrew moved into the ChatGPT Business workspace named Collect Valor, with his existing projects visible. The desktop usage menu shows a 5-hour allowance and the generic text “To get more access, contact your admin.” Earlier guidance claiming this proves Andrew is not an owner was not verified and should not be repeated. The account-role/usage-control issue is unresolved. Do not give guessed UI paths; the only verified official guidance is that Business workspace owners purchase credits at Workspace settings > Billing, while admins can manage limits. Andrew intends to return later.
- Latest local work, uncommitted: implemented a six-hour shared TCG price cache. It uses `tcg_price_cache`, keyed by the exact TCG card ID and printing, so later scans/price reads reuse the stored price rather than calling TCG again. Added required `create table` SQL to `Sports by Josh Hart/schema.sql`. The cache fails open (the app continues using TCG if the table is absent/unavailable). All 29 tests passed. Supabase SQL must be run before the cache can take effect; do not claim it is live until Andrew does that. Await commit/push authorization.
- Latest local work, uncommitted: direct TCG identity resolver now fetches the canonical TCG card record after matching, captures both its card ID and TCGplayer product ID, then fetches its exact printing price. The TCGplayer ID is retained with a saved collection card; `schema.sql` adds that optional column. All 30 tests passed. The required Supabase SQL has not been confirmed as run, and no commit/push is authorized yet.
