...
const TESTER_PASS_CODE = 'CV-TEST-40';
const TESTER_PASS_EXPIRES_AT = '2026-10-18T23:59:59.000Z';
...
export default async (req) => {
  ...
  const configuredCode = TESTER_PASS_CODE;
  const expiresAt = Date.parse(TESTER_PASS_EXPIRES_AT);
  if (!secret) return json({ error: 'Tester access is not configured.' }, 503);
